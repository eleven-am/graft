...
func (n *AggressiveTestNode) CanStart(ctx context.Context, state []byte, config []byte) bool {
	atomic.AddInt64(&n.canStartCount, 1)
	// Simulate random canStart failures
	return atomic.LoadInt64(&n.canStartCount)%7 != 0
}

func (n *AggressiveTestNode) Execute(ctx context.Context, state []byte, config []byte) (*domain.NodeResult, error) {
	n.mutex.Lock()
...
func (n *FastTestNode) Execute(ctx context.Context, state []byte, config []byte) (*domain.NodeResult, error) {
	// Very fast execution - just return success
	return &domain.NodeResult{
		GlobalState: map[string]interface{}{"completed": true},
		NextNodes:   []domain.NextNode{}, // Terminal node
	}, nil
}

func (n *FastTestNode) CanStart(ctx context.Context, state []byte, config []byte) bool {
	return true
}

// TestNodeRegistry implementation for aggressive testing
type TestNodeRegistry struct {
	nodes map[string]domain.NodePort
	mutex sync.RWMutex
}

func NewTestNodeRegistry() *TestNodeRegistry {
	return &TestNodeRegistry{
		nodes: make(map[string]domain.NodePort),
	}
}

func (r *TestNodeRegistry) RegisterNode(node domain.NodePort) error {
	r.mutex.Lock()
	defer r.mutex.Unlock()
	r.nodes[node.GetName()] = node
	return nil
}

func (r *TestNodeRegistry) GetNode(nodeName string) (domain.NodePort, error) {
	r.mutex.RLock()
